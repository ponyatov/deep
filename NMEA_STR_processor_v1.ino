...
if(GPSDataOK == 3) 
  ...
  for(int recongi = 0 ;recongi< 8;recongi++)
    {
     if (NMEAdataBufer[recongi]==NMEArecongGPS[recongi])  comparisonGPS++;
 
    }
  if(comparisonGPS==7) GPSDataOK =2;
 ...
 return  GPSDataOK ;
  ...
  String readValString(int carriage , bool cariageflag) 
  {
    ...
    while(rot) 
    {
     ...
     if ( countchar > 15 ) 
        {
        if(cariageflag == true ) GPScomparisonErRecData++ ; 
        if(cariageflag == false ) SDcomparisonErRecData++ ;  
         break ;
        }
 ...
 if(SDDataOK == 3) 
  {
   for(int recongi = 0 ;recongi< 8;recongi++)
      {
       if (NMEAdataBufer[recongi]==NMEArecongSD[recongi])  comparisonSD++;
 
      }
   if(comparisonSD==7) SDDataOK =2;
